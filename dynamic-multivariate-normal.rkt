...
;;
;; dynamic-multivariate-normal.rkt -
;;   Untyped Racket variant of multivariate-normal.rkt.  Literally the same
;;   code, but switched to #lang racket and overrided Typed Racket features
;;   using macros. 
;;
;; Modified by: Ronald Garcia <rxg@cs.ubc.ca>
;; Date: December 10, 2018
;;
;; Change Log:
;; December 10, 2018:
;; - Switched from Typed Racket to Racket (using macros to suppress types).
;; - Replaced Cholesky Decomposition with an implementation from Rosetta Code.
;;   I suspect that the one here had bit-rotted since 0-element matrices are
;;   verboten by contract:  I guess that wasn't previously the case.
;;


(require math
         "../cholesky.rkt"
         (rename-in "main.rkt" [cholesky broken-cholesky])
...
(define-syntax for/matrix:
  (syntax-rules ()
    [(_ m n b* : type e) (for/matrix m n b* e)]))
